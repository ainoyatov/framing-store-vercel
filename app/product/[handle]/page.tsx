      ...
      <div className="mx-auto px-4 max-w-screen-2xl ">
        <div className="flex flex-col rounded-lg border border-neutral-200 bg-white p-8 dark:border-neutral-800 dark:bg-black md:p-12 lg:flex-row lg:gap-8">
          <div className="h-full w-full basis-full lg:basis-4/6">
            <ShopifyGallery
              images={product.images.map((image: ImageProps) => ({
                src: image.url,
...
async function RelatedProducts({ id }: { id: string }) {
  ...
  return (
    <div className="py-8">
      ...
      <ul className="flex h-64 w-full gap-4 overflow-x-auto pt-1">
        {relatedProducts.map((product) => (
          <li
            key={product.handle}
            className="aspect-square min-h-fit w-full flex-none min-[475px]:w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5"
          >
